...
CREATE MATERIALIZED VIEW public.v_trips_minutely AS

SELECT
    provider_id,
    provider_name,
    date_trunc('minute', timezone('America/Los_Angeles'::text, end_time)) AS end_minute_local,
...
FROM public.trips
GROUP BY provider_id, provider_name, end_minute_local
ORDER BY end_minute_local DESC, provider_name
